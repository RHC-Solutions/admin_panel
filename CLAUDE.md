...
- **Deeper docs**: [docs/TECHNICAL.md](./docs/TECHNICAL.md), [docs/TROUBLESHOOTING.md](./docs/TROUBLESHOOTING.md), [docs/CHANGELOG_PERFORMANCE.md](./docs/CHANGELOG_PERFORMANCE.md), [docs/audits/AUDIT_SEO_2026-05-18.md](./docs/audits/AUDIT_SEO_2026-05-18.md) (deep SEO audit + phased plan; Phase 1 complete).

## What this repo is (read first — it changes how to read the rest)

This repo (`RHC-Solutions/rhc-cms`) is the **embeddable CMS admin only** — `src/app/{admin,api,pack-preview}`, the shared `src/lib`/`src/components`, the installer, and the design-pack/provisioning/OODA subsystems. It is **not** a full website: there are **no** public pages, no `cms-data/`, no `src/app/robots.ts` / `sitemap.ts` / `(public)/*` here. Hosts pull this in as a `vendor/admin-panel` git submodule. "WordPress for the Node/Next/Postgres stack." When you edit shared code, assume it runs **inside an arbitrary host**, not just one site — don't hardcode RHC paths/domains.

> **Host vs. panel — important for the rest of this file.** The "Build / deploy", "Where things live", "Analytics architecture", "Brand", "SEO", and "Common debug entry points" sections below describe the **reference host** this panel was extracted from and still powers (`rhcsolutions.com`). Paths they mention — `src/app/(public)/*`, `src/app/robots.ts`, `src/app/sitemap.ts`, `cms-data/*.json`, `src/components/cms/*` — live in the **host site**, not in this repo, so those links are dangling *here* by design. Treat them as the canonical example of the conventions a host should follow when embedding the panel.

### Platform subsystems (this repo's own code — read before touching them)

- **Distribution.** Hosts embed via `npx github:RHC-Solutions/rhc-cms init`; imports use the `@adminpanel/*` alias (→ `src/*`); routes are thin re-export wrappers generated by `scripts/install-into-site.mjs`; auth is `adminAuthGate` composed into the host's `middleware.ts`. CLI: `bin/admin-panel.mjs` (`init`/`update`/`apply-pack`, `--static-site`). Don't hardcode RHC paths/domains into shared lib code. Full picture: [README.md](./README.md), [INSTALL.md](./INSTALL.md).
- **Design packs** (`src/lib/design-pack/*`, [docs/DESIGN_PACKS.md](./docs/DESIGN_PACKS.md)). `POST /api/cms/design-pack/apply` auto-detects two kinds: **CMS-block** packs (`pack.json`, decomposed into blocks) and **static-site** packs (no `pack.json`; finished `.html` + `assets/`, ingested as `staticpage` CMS pages and served **verbatim**). Packs carry *design only* — the importer **hard-rejects** `secrets.json`/`users.json`/`seo.json`/`cms.db`/`.env*` and drops identity keys; `{{token}}` placeholders are filled by the wizard. Path-safety guards on every zip entry. Static-pack hosts serve pages via the scaffolded `app/[[...slug]]/route.ts`; in-panel preview is `/pack-preview/<slug>`. **Never commit an example/test pack into this repo** — it's the consumer, not a pack store.
- **Provisioning wizard** (`src/app/admin/setup/page.tsx`, `POST /api/cms/setup/provision`). First-run flow: design → configure (domain, Brevo/SMTP, Cloudflare token + DNS automation `src/lib/cloudflare/dns.ts`, live validation `src/lib/integrations/test.ts`) → account → MFA. The first-run gate is **fail-CLOSED** `adminExists()` in `src/lib/auth/setup-gate.ts` (only ENOENT = first-run). Provision writes domain → `.env.local` but **never overwrites `NEXTAUTH_URL`**; `src/lib/env.ts setEnvValue` rejects CR/LF + `$`-injection.
- **OODA self-improvement loop** (`src/lib/ooda/*`, `/admin/automation`). Observe (daily audits) → Orient → Decide → Act. **Auto-applies only the `SAFE_ACTION_TYPES` allowlist** (`revalidate`/`sync-seo`/`scan-media`); everything else is proposed. **Dry-run by default** — don't widen the allowlist or flip the default without cause.
- **CloudPanel module** (`src/lib/cloudpanel/*`, `/admin/cloudpanel`, [docs/CLOUDPANEL.md](./docs/CLOUDPANEL.md)). Manages the CloudPanel server the panel is installed on. CloudPanel has **no REST API** and every `clpctl` verb needs root, so reads open CloudPanel's own SQLite DB **read-only** (schema-introspecting — never hardcode its table/column names) and writes go through an **allowlisted** `clpctl` call via a root-owned wrapper (`scripts/cloudpanel/rhc-clpctl`) + sudo. **Off by default**; destructive verbs need *both* the panel switch *and* a root-owned marker file `/etc/rhc-clpctl-allow-destructive`, plus a retyped target. Never use a shell (`execFile` + argv only), never emit a value as its own argv token (`--flag=value` single element), never widen the sudoers rule to `clpctl` itself — the wrapper is the security boundary, the TS validation is only a usability layer.
- **DB driver seam** (`src/lib/cms/db/{driver,sqlite,postgres,index}.ts`). `cmsDb` is driver-backed and async; **SQLite is the zero-config default**, Postgres opt-in via `DATABASE_URL`/`DB_DRIVER`. Keep SQL portable: `?` placeholders (the PG driver translates to `$n`), **quote camelCase columns** (`"showInFooter"`/`"createdAt"`…) so PG doesn't fold them, coerce `COUNT` with `Number()`, use `ON CONFLICT … DO UPDATE SET x = excluded.x`. Guard file-only ops (`backup`/`restore`/audit WAL checkpoint) with `isSqlite()` — they no-op under PG (`pg_dump` is the PG backup path). Don't re-add import-time/sync DB init; `ready()` is the lazy memoized init.
...
- **Homepage/Contact page copy** (value-prop, bookingUrl, stats band, CTA band, contact-page sections) is edited by `SiteCopyEditor` at the top of **`/admin/pages`** (it moved out of Settings — it's page content, not site config). It still saves to `settings.json` (`brand`/`stats`/`homeContent`/`contactContent`/`bookingUrl`) via `/api/cms/settings`, sending **full deep-merged objects** (the settings PUT top-level-merges) so unexposed sub-keys (`brand.about`, `stats.support`, …) survive. The Settings General save deliberately **omits** these keys so it can't clobber copy edited in Pages.
...
## Settings, integrations & updates (2026-06 consolidation)

- **One Settings hub.** `/admin/settings` is tab-driven: **General · Integrations · Advanced**. The Integrations tab renders the shared `src/components/admin/IntegrationsPanel.tsx` (extracted from the old standalone page) off the `INTEGRATIONS` catalog (`src/lib/integrations.ts`). The old standalone routes **308-redirect** into it: `/admin/integrations`, `/admin/analytics/setup`, `/admin/cloudflare/setup` → `/admin/settings?tab=integrations`. `/admin/cloudflare` (operational dashboard) and `/admin/settings/environment` (infra `.env.local`, linked from Advanced) stay.
- **Single credential store.** Secrets live in `cms-data/secrets.json` via `setSecrets`/`getSecret` — the consolidated UI routes **every** credential there (Cloudflare API token + account ID and the GA4 service-account identity fields are now in the catalog). `MANAGED_SECRET_KEYS` auto-derives from the catalog, so adding a field there auto-allow-lists it for the save endpoint **and** the wizard. Avoid re-introducing `.env.local` secret writers (the old Analytics/Cloudflare setup pages caused a secrets.json-vs-.env split-brain where secrets.json silently won). Genuinely build-time/public values (`NEXT_PUBLIC_*`, `NEXTAUTH_*`, `DATABASE_URL`) stay in `.env.local` via the Environment editor.
- **Nav + search share one source.** `src/lib/admin-nav.ts` is the single nav tree consumed by both `AdminShell` (sidebar, via a `NAV_ICONS` map) and `src/lib/admin-search.ts`. Search also indexes **every integration + credential field** from the catalog. Add pages to `admin-nav.ts` (not to two lists); `hidden: true` = searchable but not in the sidebar.
- **Self-service account.** Top-bar user block links to `/admin/account` (own email/password/2FA). `/api/cms/account` is **strictly self-scoped** to the JWT email and never accepts `role`/`status` — don't loosen it into the admin-only `/api/cms/users`.
- **Media.** `seedBrandMedia()` (`src/lib/cms/media-scan.ts`) surfaces the bundled logo/favicon/OG image so `/admin/media` isn't empty on a fresh deploy (lazy-seeded on an empty GET + during provision). `src/components/admin/MediaPicker.tsx` is the reusable browse-the-library field (used by SEO OG image; adopt it for new image fields instead of a raw path input).
- **First-run wizard** (`/admin/setup`) collects all integrations via a catalog-driven accordion (merged into the provisioned `secrets`), and provision seeds brand media + `seo.json` from the identity step.
